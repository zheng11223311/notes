...
if __name__ == '__main__':
    ...
    mask=cv2.drawContours(mask,contours,index[-1],(255,255,255),thickness=-1) #绘制轮廓,绘制倒数第二个轮廓index[-2],thickness -1 轮廓内部都会被填充为白色(255,255,255)
    # 2、函数的调用形式
    ...
    for x, y, w, h in faces:
        ...
        mask3=(mask2-255)*255  #黑白颜色相互转换

        head_gray2=cv2.resize(head,(w,h))
        face=img[y:y+h,x:x+w]
        face2=np.zeros_like(face,dtype=np.uint8)
        # face=cv2.bitwise_and(face,face,mask=mask2)  #与运算,mask的黑色地方没有进行与运算,直接使用mask 的黑色部分遮挡,mask 白色部分进行了与运算
        head3=cv2.bitwise_and(head_gray2,head_gray2,mask=mask2)
        face=cv2.bitwise_and(face,face,mask=mask3)
        face=cv2.bitwise_or(head3,face) #没有给mask,为计算所有
        img[y:y+h,x:x+w]=face

        # face=cv2.bitwise_not(face,face,mask=mask2)  #非运算
        # face=cv2.bitwise_or(face,face,mask=mask2)  #或运算
        # face=cv2.bitwise_xor(face,face,mask=mask2)  #异或运算
