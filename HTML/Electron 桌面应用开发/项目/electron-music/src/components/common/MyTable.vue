<template>
  <div id="table">
    ...
    <ul>
      <li v-for="(item, index) in currentData" :key="item.MUSICRID"
        @click="sendMusicId(item.MP3RID.split('_')[1], item.MKVRID.split('_')[1], item.SONGNAME, item.ARTIST)">
        <p title="">{{ index + 1 }}</p>
        <p>操作</p>
        <p class="mv">
          <span v-html="item.SONGNAME" :title="item.SONGNAME"></span>
          <span class="iconfont icon-wmv" v-if="item.MKVRID.split('_')[1] != 0" title="播放MV"
            @click="sendMvId($event, item.MP3RID.split('_')[1])"></span>
        </p>
