...
function zjs_pos_p(str, t) {
    //坐标表达方式转换。
    if (typeof(str) == "string") {
        let num = parseFloat(str);
        if (str.indexOf("%") >= 0) {
            if (t == "x") {
                return num / 100 * constData.screenWidth;
            } else if (t == "y") {
                return num / 100 * constData.screenHeight;
            } else {
                return num / 100;
            };
...
function zjs_pos_location(obj) {
    ...
    /*
        ...
        */
    let x = zjs_pos_p(obj.x, "x");
    let y = zjs_pos_p(obj.y, "y");

    if (obj.xOffset) {
        x += zjs_pos_p(obj.xOffset, "x");
    };
    if (obj.yOffset) {
        y += zjs_pos_p(obj.yOffset, "y");
    };
...
function zjs_pos_image(obj) {
    ...
    //只实现了简单的区域找图。
    if (obj.xOffset) {
        p.x += zjs_pos_p(obj.xOffset, "x");
    };
    if (obj.yOffset) {
        p.y += zjs_pos_p(obj.yOffset, "y");
    };
    img.recycle();
