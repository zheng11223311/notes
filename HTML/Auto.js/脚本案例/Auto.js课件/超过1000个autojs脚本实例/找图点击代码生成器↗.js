...
var isSaveToDir=false,saveToDir;
...
function clipToCode() {
    if (findImageDataAry.length) {
        ...
        if(isSaveToDir){
            ...
            toastLog("已保存文件" + findImageDataAry.length);
        }else{
            setClip(str);
...
window.canvas.on("draw", function(canvas) {
    ...
    if (MainImg) {
        ...
        //找图数组数量
        canvas.drawText(String(findImageDataAry.length), 0, 75, paint2);
