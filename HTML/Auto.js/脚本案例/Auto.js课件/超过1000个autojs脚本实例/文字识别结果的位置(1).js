...
var words_result = BaiDu_ocr(img, true).words_result;
    ...
    for (var i = 0; i < words_result.length; i++) {
        ...
        obj.words2=Baidu_To(obj.words,"auto","zh");
    };
...
window.canvas.on("draw", function(canvas) {
    ...
    canvas.drawRect(0+cw, 0, device.width+cw, device.height, paint1);
    for (var i = 0; i < words_result.length; i++) {
        var obj = words_result[i];
        ...
        canvas.drawRect(rect.left+cw, rect.top, rect.left + rect.width+cw, rect.top + rect.height, paint1);
        canvas.drawText(obj.words2, rect.left+cw, rect.top + rect.height - rect.height * 0.15, paint2);
    };
});
