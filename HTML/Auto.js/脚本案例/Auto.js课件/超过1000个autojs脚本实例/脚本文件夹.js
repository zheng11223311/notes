        ...
        <list id="list" layout_weight="1">
            <card w="*" margin="5" cardCornerRadius="10dp" cardElevation="1dp" >
                <horizontal gravity="center_vertical" bg="{{getBG(file)}}">
                    ...
                    <checkbox id="done"h="*" bg="#33777777" margin="4" text="选中" checked="{{done}}" visibility="{{isCanLook}}"/>
                </horizontal>
            </card>
        </list>
        <horizontal w="*">
            <button id="exit_" layout_weight="1" text="退出" />
            <button id="select_" layout_weight="1" text="选择" />
...
var StartDirFile = new java.io.File("/sdcard");

var CurrentDirFile = new java.io.File(StartDirFile);
UpListDate(CurrentDirFile);

ui.select_.click(function(v) {
    if (v.getText() == "选择") {
        v.setText("取消");
        UpListDate(CurrentDirFile, true);
    } else {
        v.setText("选择");
        UpListDate(CurrentDirFile);
    };
