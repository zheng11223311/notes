...
flo.setSize(device.width / 10, device.width / 15);
flo.exitOnClose();
sleep(500);
...
var xy0 = [0, 0],
    ...
    longPressTime = 600,
    longPressThreads = "";
flo.setPosition(floatyxy0[0], device.height * 13 / 32);
flo.setSize(0, 0);
    ...
    try {
        ...
        switch (currentActivity()) {
            ...
            case "com.tencent.mm.ui.LauncherUI":
                if (text("发送").enabled(true).findOne(200) != null) {
                    if (!floatyState) {
                        flo.setSize(device.width / 10, device.height / 25);
                        floatyState = true;
                    }
                } else {
                    if (floatyState) {
