...
ui.canvas.on("draw", function(canvas) {
   ...
   matrix.setRectToRect(new android.graphics.RectF(-b_e,0,mBitmap.getWidth()+b_e,mBitmap.getHeight()), new android.graphics.RectF(0,0,w,h), android.graphics.Matrix.ScaleToFit.CENTER);
...
var ST={
    ...
    y:start.y?start.y:start.y+1
};
var EN={
    x:end.x==g_w-1?g_w-2:end.x,
    y:end.y==g_h-1?g_h-2:end.y
};
...
mCanvas.drawRect(start.x * b_e, start.y * b_e, (start.x + 1) * b_e, (start.y + 1) * b_e, paint1); //左    
mCanvas.drawRect(end.x * b_e, end.y * b_e, (end.x + 1) * b_e, (end.y + 1) * b_e, paint1); //左    
DFS(EN);
    toastLog("迷宫生成完成");
...
function DFS(xy){
    ...
    if(fx.length){
        ...
        mCanvas.drawRect((x+(fxy.x-x)/2) * b_e, (y+(fxy.y-y)/2) * b_e, ((x+(fxy.x-x)/2) + 1) * b_e, ((y+(fxy.y-y)/2) + 1) * b_e, paint2); //左
        DFS(fxy);
    };
    //为了看得直观一点。加个延时。
    //sleep(1);
