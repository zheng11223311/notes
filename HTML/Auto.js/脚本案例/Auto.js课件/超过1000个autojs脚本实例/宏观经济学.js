...
while (true) {//死循环
x=0;xk=0;if(kz!=0){akzt("QQ:唐僧,点击确定开始执行,系数越大跳的越远,把此悬浮窗拖到左下角");kz=0;zt0=(new Date()).getTime();
akzt2();//暂停控制
}//x坐标0，xk坐标相同次数
while(true){//死循环找人物棋子
...
presso(963*n+random(-10,10),1588*v+random(-15,15),time);//随便找一点可以操作的点进行精确时间点击
sleep(100);window.setPosition(-1000,-1000);sleep(500);//延时越长越稳定//点击完延时0.x秒，隐藏悬浮窗，避免挡截图
if((new Date()).getTime()-zt0>30000){//已执行100秒
if(files.create("/sdcard/"+zzz+"autojs.666")){
    sc("已执行:"+(0.5+zzz*4)+"分钟\n"+sbxx());}
zt0=(new Date()).getTime()+210000;
zzz+=1;
}
}//死循环，永不结束
