...
ui.layout(
    <vertical>
        ...
        <horizontal h="auto">
            ...
            <button id="but_b" layout_weight="1" h="auto" text="保存图片"/>
...
function canvasDraw(canvasView) {
    ...
    this.toImage=function(){
        var canvas=new Canvas(canvasView.getWidth(),canvasView.getHeight());
        this.viewDraw(canvas);
    ...
    this.viewDraw=(canvas) => {
        ...
        if (this.TouchPointStart.length) {
            for (let i = 0; i < this.TouchPointRecord.length; i++) {
                ...
                //canvas.drawTextOnPath("我爱你",this.TouchPointRecord[i],0,0,this.paint);
            };
            return
...
//    this.paint.setShader(new this.gr.LinearGradient(0,1920,0,0,this.toJavaArray("int",[colors.RED,colors.GREEN,colors.BLUE,colors.RED]),null,this.gr.Shader.TileMode.CLAMP));

canvasView.on("draw", this.viewDraw);
