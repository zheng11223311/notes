...
var w = floaty.window(
    <vertical id='box'  bg="#ffffffff">
        <horizontal>
                <text >攻击第几行怪:  </text>
                <text id="attackEmey">1</text>
                <button id="attackEmeyAdd" >+</button>
                <button id="attackEmeyIncrement" >-</button>
        </horizontal>
        <horizontal>
                <text >低血时吃第几行回血药瓶:  </text>
                <text id="recoveryHP">1</text>
                <button id="recoveryHPAdd" >+</button>
                <button id="recoveryHPIncrement" >-</button>
        </horizontal>
        <horizontal>
                <text >低精时吃第几行回血精瓶:  </text>
                <text id="recoveryHV">4</text>
                <button id="recoveryHVAdd" >+</button>
                <button id="recoveryHVIncrement" >-</button>
        </horizontal>
        <horizontal>
                <text >异常登出时选择第几行:  </text>
                <text id="loginoutNum">1</text>
                <button id="loginoutNumAdd" >+</button>
                <button id="loginoutNumIncrement" >-</button>
        </horizontal>
        <button id="back" >结束战斗</button>
        ...
        <text text="提示: 点击背景进行缩放!" textColor="#ff0000"/>
...
w.start.on('click',()=>{
    // ui 内不能执行太多函数,
    // 这里用来修改变量,让外部函数根据变量决定是否执行其他操作
    // console.log(w.attackEmey.getText());
    // console.log(w.recoveryHP.getText());
    // console.log(w.recoveryHV.getText());
    attackEmey= parseInt(w.attackEmey.getText())
    recoverHP= parseInt(w.recoveryHP.getText())
    recoverHV= parseInt(w.recoveryHV.getText())
    isUiStart=true
    // 只有一次是正常登入
    isNorlogin=true
    // 赋予检测截图权限
...
//攻击
w.attackEmeyAdd.on('click',()=>{
    let num=parseInt(w.attackEmey.getText())
    num++
    w.attackEmey.setText(String(num))
    // console.log(num);
})
w.attackEmeyIncrement.on('click',()=>{
    let num=parseInt(w.attackEmey.getText())
    num--
    if(num<=0){
        num=1
...
// 回血
w.recoveryHPAdd.on('click',()=>{
    let num=parseInt(w.recoveryHP.getText())
    num++
    w.recoveryHP.setText(String(num))
    // console.log(num);
})
w.recoveryHPIncrement.on('click',()=>{
    let num=parseInt(w.recoveryHP.getText())
    num--
    if(num<=0){
        num=1
...
// 回精
w.recoveryHVAdd.on('click',()=>{
    let num=parseInt(w.recoveryHV.getText())
    num++
    w.recoveryHV.setText(String(num))
    // console.log(num);    
})
w.recoveryHVIncrement.on('click',()=>{
    let num=parseInt(w.recoveryHV.getText())
    num--
    if(num<=0){
        num=1
...
// 异常登出选择角色
w.loginoutNumAdd.on('click',()=>{
    let num=parseInt(w.loginoutNum.getText())
    num++
    w.loginoutNum.setText(String(num))
    // console.log(num);    
})
w.loginoutNumIncrement.on('click',()=>{
    let num=parseInt(w.loginoutNum.getText())
    num--
    if(num<=0){
        num=1
    ...
    if(noPort){
        ...
        if(popRole){
            ...
            let thread=threads.start(function(){
                ...
                // 参数: 颜色,第几个角色
                sleep(2000)
                dealBugPopRole(popRole,loginoutNum)
