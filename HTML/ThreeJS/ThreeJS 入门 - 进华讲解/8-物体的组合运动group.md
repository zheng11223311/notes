...
scene ->group -> cube1 /cube2 /cube3



```js
 // 创建组合,将三个立方体作为一个组,一起绕一个轴旋转,而不是绕各自的轴旋转
        // 将三个立方体加入到group 中,
        // group 加入到scene 中
        // 可以创建多个组合,一个组也可以像立方体一样加入到另一个组中
        const group=new THREE.Group()
        //...创建cube...
          group.add(cube) //加入到组中
		  group.add(cube1) //加入到组中
		  group.add(cube2) //加入到组中
...
`ThreeJS 入门 - 进华讲解\代码\7-物体的组合运动group.html`
...
<body>
    ...
    <script>
        ...
        const material1 = new THREE.MeshBasicMaterial({
            color: 0xffffff * Math.random() //十六进制的随机颜色
        })
        let cube1 = new THREE.Mesh(geometry1, material1)
        // scene.add(cube1) //加入到场景中
        group.add(cube1) //加入到组中
        // cube1.add(axes)
